...
#pragma endregion
	...
	private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
			...
			_for2(x, first_order->Width) {
				...
				if (new_y >= 0 && y < bd_ori->Height && new_x >= 0 && new_x < bd_ori->Width - 1) {
					...
					_for2(s, 3) {
						...
						double add = (*(ori_ptr + n)) * (f_k - r) + (*(ori_ptr + n + 3)) * r;
						first_order_ptr[s] = (Byte)(add / f_k);
					}
				}
				else {
					_for2(s, 3) {
		...
		int x1, y1;
		float temp[3];
		_for3(y, 0, (bilinear->Height - k)) {
			_for3(x, 0, (bilinear->Width - k)) {
				f_y = y / k;
				f_x = x / k;
				y1 = y / k;
				x1 = x / k;
				...
				bilinear_ptr += (int) (y * bd_bilinear->Stride + x * 3);
				_for2(s, 3) {
